...
The reset and refresh scripts also require Laravel to resolve `APP_ENV=local`.# Client Lifecycle Operations
...
Populate the client URL, database identity/credentials, and client-specific namespaces, then set only the selected key in the platform root `.env`:
